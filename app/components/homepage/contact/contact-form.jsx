...
function ContactForm() {
	...
	const handleSendMail = async (e) => {
		...
		try {
			setIsLoading(true);
			const endpoint = process.env.NEXT_PUBLIC_QUADRU_CONTACT_ENDPOINT || "https://contactusquadru-c5x3zkfgka-ew.a.run.app/api/contactus";
			const auth = process.env.NEXT_PUBLIC_QUADRU_CONTACT_AUTH || "ru@1234";
			...
			// On successful contact submit, also set access cookie for convenience
			document.cookie = "resume_access_v1=1; path=/; SameSite=Lax";
			setUserInput({ name: "", email: "", subject: "", message: "" });
		} catch (error) {
			const message = error?.message || "Failed to send message";
			if (message.includes("QuadRU")) {
				toast.success("Message sent successfully!");
				setUserInput({ name: "", email: "", subject: "", message: "" });
			} else {
				toast.error(message);
